  ...
  useEffect(() => {
    ...
    function makeSeries(name: any, fieldName: any) {
      ...
      series.columns.template.events.on('click', (ev) => {
        ...
        const find = depotTypeChart.find((emp: any) => emp.category === categorySelect);
        const typeSelect = find?.value;
        const selectedStatus: number | null = fieldName === 'comp' ? 4 : 1;

        const defaultExpression = 'Type = ' + typeSelect + ' AND ' + 'Status = ' + selectedStatus;
        const expression =
          "Building_Names = '" +
          props.depotname +
          "'" +
        ...
        view.when(() => {
          view.whenLayerView(depotBuildingStructureLayer).then((layerView: any) => {
            depotBuildingStructureLayer.queryFeatures(query).then((results: any) => {
            ...
            layerView.filter = new FeatureFilter({
              where: !props.depotname ? defaultExpression : expression,
            });
          });
        });
      });
